...
export const usePayflowBusdPrice = (): Price | undefined => {
  const payflowBusdPrice = useBUSDPrice(tokens.payflow)
  return payflowBusdPrice
}
...
export const useBUSDPayflowAmount = (amount: number): number | undefined => {
  const payflowBusdPrice = usePayflowBusdPrice()
  if (payflowBusdPrice) {
    return multiplyPriceByAmount(payflowBusdPrice, amount)
  }
  return undefined
}
