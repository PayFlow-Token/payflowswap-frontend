...
import { usePayflow, useTokenContract } from './useContract'
...
export const useTotalSupply = () => {
  const { reader: payflowContract } = usePayflow()
  const { data } = useSWRContract([payflowContract, 'totalSupply'], {
...
export const useGetPayflowBalance = () => {
  const { balance, fetchStatus } = useTokenBalance(tokens.payflow.address)
